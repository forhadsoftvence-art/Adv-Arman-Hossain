...
(async()=>{
 ...
 // Hero CTA contrast pair, min 54px height, header CTA untouched, 5+ experience, flag rows.
 const heroGold=page.locator('.hero-actions .btn-gold'); const heroCall=page.locator('.hero-actions .btn-outline');
 const heroStyle=await heroGold.evaluate(e=>{const s=getComputedStyle(e);const box=e.getBoundingClientRect();return{height:box.height,bg:s.backgroundColor,color:s.color,border:s.borderTopWidth}}) ;
 assert.ok(heroStyle.height>=54,'hero gold CTA height '+heroStyle.height);
 assert.equal(heroStyle.bg,'rgb(220, 185, 104)');
 assert.equal(heroStyle.color,'rgb(10, 20, 32)');
 assert.ok(heroStyle.border!=='0px','hero gold CTA border');
 const callStyle=await heroCall.evaluate(e=>{const s=getComputedStyle(e);return{height:e.getBoundingClientRect().height,bg:s.backgroundColor,color:s.color}});
 assert.ok(callStyle.height>=54,'hero call CTA height '+callStyle.height);
 assert.equal(callStyle.bg,'rgb(250, 249, 246)');
 assert.equal(callStyle.color,'rgb(10, 20, 32)');
 assert.ok((await heroGold.evaluate(e=>getComputedStyle(e).fontWeight))>='600');
 const headerCtaStyle=await page.locator('.header-cta').evaluate(e=>({h:e.getBoundingClientRect().height,bg:getComputedStyle(e).backgroundColor}));
 assert.ok(headerCtaStyle.h>=42&&headerCtaStyle.h<54,'header CTA untouched min-height '+headerCtaStyle.h);
 assert.equal(await page.locator('.badge-experience strong').textContent(),'৫+');
 assert.equal(await page.locator('.stat [data-count="5"]').textContent(),'৫+');
 assert.ok(['flex','inline-flex'].includes(await page.locator('.country-note').evaluate(e=>getComputedStyle(e).display)),'country-note flex row');
 assert.equal(await page.locator('.country-note').evaluate(e=>getComputedStyle(e).alignItems),'center');
 assert.equal(await page.locator('.country-note').evaluate(e=>getComputedStyle(e).gap),'8px');
 assert.equal(await page.locator('.country-note > .country-label').textContent(),'গর্বিতভাবে বাংলাদেশের সেবায়');
 assert.ok(['flex','inline-flex'].includes(await page.locator('.footer-country').evaluate(e=>getComputedStyle(e).display)),'footer-country flex row');
 assert.equal(await page.locator('.footer-country > .country-label').textContent(),'গর্বিতভাবে বাংলাদেশের সেবায়');
 assert.equal(await page.locator('.about-copy').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(255, 253, 249)');
 // Netlify Drawer default-hide logic (localhost untouched; deploy-preview hostname gets one hidden param).
 assert.ok(!new URL(page.url()).searchParams.has('ntl-drawer-state'),'localhost toolbar untouched');
 const ntl=await page.evaluate(()=>window.__netlifyPreviewHiddenUrl('https://deploy-preview-12--adv-arman-hossain.netlify.app/?a=1#faq'));
 assert.equal(ntl,'https://deploy-preview-12--adv-arman-hossain.netlify.app/?a=1&ntl-drawer-state=hidden#faq');
 assert.equal(await page.evaluate(()=>window.__netlifyPreviewHiddenUrl('https://deploy-preview-12--adv-arman-hossain.netlify.app/?ntl-drawer-state=visible')),null);
 assert.equal(await page.evaluate(()=>window.__netlifyPreviewHiddenUrl('https://deploy-preview-12--adv-arman-hossain.netlify.app/#ntl-drawer-state=hidden')),null);
 assert.equal(await page.evaluate(()=>window.__netlifyPreviewHiddenUrl('https://adv-arman-hossain.netlify.app/')),null);
 assert.equal(await page.evaluate(()=>window.__netlifyPreviewHiddenUrl('http://127.0.0.1:8000/')),null);
 assert.equal(await page.evaluate(()=>window.__netlifyPreviewHiddenUrl('https://deploy-preview-12--adv-arman-hossain.example.com/')),null);
 console.log('PASS: hero CTA gold/cream + 54px, header CTA unchanged, 5+ badge/counter, flag inline-flex rows, about surface, Netlify toolbar guard');
